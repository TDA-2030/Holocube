...
bool RgbPlayDocoder::video_play_screen(void)
{
    ...
    if (m_isUseDMA)
    {
        ...
        screen.pushImageDMA(0, 0, 240, 60, (uint16_t *)dst, nullptr);

        dst = m_displayBufWithDma[1];
        m_pFile->read(dst, MOVIE_BUFFER_SIZE);
        screen.pushImageDMA(0, 60, 240, 60, (uint16_t *)dst, nullptr);

        dst = m_displayBufWithDma[0];
        m_pFile->read(dst, MOVIE_BUFFER_SIZE);
        screen.pushImageDMA(0, 120, 240, 60, (uint16_t *)dst, nullptr);

        dst = m_displayBufWithDma[1];
        m_pFile->read(dst, MOVIE_BUFFER_SIZE);
        screen.pushImageDMA(0, 180, 240, 60, (uint16_t *)dst, nullptr);
