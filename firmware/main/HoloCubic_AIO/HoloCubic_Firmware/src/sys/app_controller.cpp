...
static TimerHandle_t xTimerEventDeal; // 事件处理定时器
...
int AppController::main_process(APP_ACTIVE_TYPE active)
{
    this->act_info = active;
    if (isRunEventDeal) {
    ...
    if (0 == app_exit_flag) {
        ...
        if (APP_ACTIVE_TYPE::APP_TURN_LEFT == active) {
        ...
        } else if (APP_ACTIVE_TYPE::APP_TURN_RIGHT == active) {
            ...
            Serial.println(String("Current App: ") + appList[cur_app_index]->app_name);
        } else if (APP_ACTIVE_TYPE::APP_GO_FORWORD == active) {
        ...
        if (APP_ACTIVE_TYPE::APP_GO_FORWORD != active) { // && UNKNOWN != active
            app_control_display_scr(appList[cur_app_index]->app_image,
                                    appList[cur_app_index]->app_name,
                                    anim_type, false);
            delay(300);
        ...
        (*(appList[cur_app_index]->main_process))(this);
    }
                           ...
                           void *ext_info)
{
    ...
    if (type <= APP_MESSAGE_MQTT_DATA) {
        ...
        ESP_LOGI(TAG, "\tEventList Size: %d", eventList.size());
    } else {
        // 各个APP之间通信的消息
        if (NULL != toApp) {
            ESP_LOGI(TAG, "[Massage]From %s To %s", String(fromApp->app_name).c_str(),  String(toApp->app_name).c_str());
            if (NULL != toApp->message_handle) {
                toApp->message_handle(from, to, type, message, ext_info);
            }
        } else if (!strcmp(to, CTRL_NAME)) {
            ESP_LOGI(TAG, "[Massage]From %s To %s", String(fromApp->app_name).c_str(),  CTRL_NAME);
